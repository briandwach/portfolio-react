const projectData = [
    {   
        name: 'Checkrūm',
        appUrl: 'https://checkrum-jshn.onrender.com',
        githubUrl: 'https://github.com/briandwach/checkrum',
        image: '/images/checkrum.png',
        alt: 'Screenshot of Checkrūm application',
        desc: 'Inspection tracker for audio video technologies in conference rooms',
        tech: 'React | MongoDB | GraphQL | JWT | date-fns | Tailwind CSS| Daisy UI'
    },
    {   
        name: 'Concert Sampler',
        appUrl: 'https://briandwach.github.io/concertsampler/',
        githubUrl: 'https://github.com/briandwach/concertsampler',
        image: '/images/concertsampler.png',
        alt: 'Screenshot of Concert Sampler application',
        desc: 'Spotify playlist generator using concert data by locale',
        tech: 'Spotify Web API | JamBase Concert API | JavaScript | Bootstrap | HTML | CSS'
    },
    {   
        name: 'JavaScript Quiz',
        appUrl: 'https://briandwach.github.io/codingassessment',
        githubUrl: 'https://github.com/briandwach/codingassessment',
        image: '/images/javascriptquiz.png',
        alt: 'Screenshot of JavaScript quiz application',
        desc: 'Timed quiz that stores high scores in the browser',
        tech: 'JavaScript | Local Storage | HTML | CSS'
    }
];

export default projectData;